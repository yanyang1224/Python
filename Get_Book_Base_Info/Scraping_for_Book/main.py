...
# 基础url和请求头
baseUrl = "http://opac.zjlib.cn/opac/"
...
async def dealPageLinks(links):
    try:
        ...
        myapp.error("异步线程出错:{0}".format(err))
...
def dealMainPage(page):
    try:
        ...
        # asyncio.run(dealPageLinks(links))

        # await dealPageLinks(links)
        # _thread.start_new_thread(dealPageLinks,(links,))
        # asyncDealPageLinks(links)
    except HTTPError as httperror:
        myapp.info(mainUrl)
