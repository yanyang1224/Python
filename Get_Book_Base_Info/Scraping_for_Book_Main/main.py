...
async def dealPage(url):
    try:
        ...
        cur.execute("insert into ResBookInfo (PageUrl,Title,Isbn,ImgUrl,Marc,CreateTime) values (%s,%s,%s,%s,%s,%s)",(url,bookTitle,bookIsbn,bookCover,bookMarc,timeStr))
        conn.commit()
        # print(bookTitle)
        # print(bookIsbn)
        # print(bookMarc)
...
def dealMainPage(page):
    try:
        changeProxy()
        mainUrl = baseUrl + "search?q=*%3A*&searchType=standard&isFacet=false&view=simple&searchWay=class&rows=100&sortWay=score&sortOrder=desc&searchWay0=marc&logical0=AND&page=" + str(page)
        print(mainUrl)
        ...
        bs = BeautifulSoup(html,'html.parser')
        trTags = bs.find('div',{'id':'resultTile'}).find_all('tr')
        for trTag in trTags:
            bookTitle = None
            pageUrl = None
            bookIsbn = None
            bookAuthor = None
            bookPublisher = None
            bookPublishDate = None
            try:
                bookTitle = str.strip(trTag.find('span',{'class':'bookmetaTitle'}).a.get_text())
            except:
                pass
            try:
                pageUrl = trTag.find('span',{'class':'bookmetaTitle'}).a.get('href')
            except:
                pass
            try:
                bookIsbn = trTag.find('img',{'class':'bookcover_img'}).get('isbn')
            except:
                pass
            try:
                bookAuthor = str.strip(trTag.find('div',{'class':'bookmeta'}).select('div:nth-of-type(2) > a')[0].string)
            except:
                pass
            try:
                bookPublisher = str.strip(trTag.find('div',{'class':'bookmeta'}).select('div:nth-of-type(3) > a')[0].string)
            except:
                pass
            try:
                bookPublishDate = str.strip(trTag.find('div',{'class':'bookmeta'}).select('div:nth-of-type(3)')[0].get_text()).split('出版日期: ')[1]
            except:
                pass
            ...
            # timeStr = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
            # cur.execute("insert into ResBookInfo (PageUrl,Title,Isbn,CreateTime,Author,Publisher,PublishDate) values (%s,%s,%s,%s,%s,%s,%s)",(pageUrl,bookTitle,bookIsbn,timeStr,bookAuthor,bookPublisher,bookPublishDate))
            # conn.commit()
